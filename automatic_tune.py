...
class swift():
	...
	def __init__(self):
		...
		#to be done
				...
				# roll, pitch, throttle
		self.Kp = [ 1026  , 34.5 , 63.2]
		self.Ki = [ 0 , 0    ,0.0826]
		self.Kd = [ 1201  , 579  , 655]
	...
	#	8. Add error_sum
		...
		time_difference = (end_time - self.current_time).to_sec()
		#print("Current: ",self.current_time,"Previous: ",self.previous_time,"dt: ",self.dt)
		...
		self.integral[0] = self.sum_error[0] * self.Ki[0] * 0.00005
		...
		print(self.current_time.to_sec(),end_time.to_sec(),time_difference)
		if abs(self.error[0] - self.error[1]) > 0.004:
			if (self.error[0] - self.error[1]) > 0:
				if abs(self.error[0] - self.error[1]) < 0.009:
						self.Ki[0] -= 1
				elif abs(self.error[0] - self.error[1]) < 0.09:
						self.Ki[0] -= 3
				else:
						self.Ki[0] -= 7
			elif (self.error[0] - self.error[1]) < 0:
				if(self.error[0] - self.error[1]) > -0.009:
						self.Ki[0] += 1
				elif(self.error[0] - self.error[1]) > -0.09:
						self.Ki[0] += 3
				else:
						self.Ki[0] += 7
		print("Ki:",self.Ki[0],"Error:",self.error[0] - self.error[1])

		self.cmd.rcRoll     = int(1500 - ((self.Kp[0] * self.error[0]*0.03) + (self.integral[0]) + (self.Kd[0] * self.derivative[0]*0.5)))
		...
		self.plus_one_line = self.error[0] - self.error[1]
