...
# import for general purpose
import json
# import LD_3114_led_detection as led
...
class DroneController():
    def __init__(self,node, camera_node):
        ...
        self.image = camera_node.create_subscription(Image,"/video_frames",self.led_detection_callback,1)
    ...
    def led_detection_callback(self, msg):
        # detect the image and assign self.image 
        self.image = self.bridge.imgmsg_to_cv2(msg)
        print("Image received")
        # print(msg)
        cv.imshow("image", self.image)
        cv.waitKey(1)
        # thresh = led.image_processing(self.image)
        # mask = led.image_masking(thresh)
        # led_clusters, clusters = led.centroid_calculation(mask)
        # print(led_clusters, clusters)
    ...
    def pid(self):
        ...
        try:
            ...
            print(f"setpoint: {self.set_points[self.set_point_current_index]}")
        ...
        print("SETPOINT INDEX : ", self.set_point_current_index) 
        print(f"flag: {roll_hower_flag}, {pitch_hower_flag}, {throttle_hower_flag}")
        # print(f"changeit{self.changeit}")
        ...
        if(self.set_point_current_index > 5):
    ...
    def publish_data_to_rpi(self, roll, pitch, throttle):
        ...
        limit_throttle = int(MAX_THROTTLE-self.landing_decrement_sum)
        if limit_throttle<1430:
            limit_throttle = 1430

        if(limit_throttle < 1350) or (self.drone_position[2]>31.7) and self.is_landing:
            self.disarm()
...
def main(args=None):
    ...
    camera_node = rclpy.create_node('image_subscriber')
    node.get_logger().info(f"Node Started")
    node.get_logger().info("Entering PID controller loop")

    controller = DroneController(node, camera_node)
    controller.arm()
